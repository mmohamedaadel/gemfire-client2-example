...
@ClientCacheApplication(name = "AccessingDataGemFireApplication2", logLevel = "info", servers = {@ClientCacheApplication.Server(host = "localhost", port = 8080)})//locators = {@ClientCacheApplication.Locator(host = "localhost", port = 44444)})
@EnableEntityDefinedRegions(basePackageClasses = Person.class)
@EnableGemfireRepositories(basePackageClasses = PersonRepository.class)
@EnablePdx

public class GemfireClient2Application {

	public static void main(String[] args) {
		SpringApplication.run(GemfireClient2Application.class, args);
	}
	...
	ApplicationRunner run(PersonRepository personRepository) {
		...
		return args -> {
            ...
            Person Madhu = new Person("Madhu Boddu", 70);
            Person Mohamed = new Person("Mohamed", 1);

            System.out.println("Before accessing data in Pivotal GemFire...");

            asList(Madhu, Mohamed).forEach(person -> System.out.println("\t" + person));

            System.out.println("Saving Alice, Bob ,Carol ,john and abraham to Pivotal GemFire...");

            personRepository.save(Madhu);
            personRepository.save(Mohamed);


            System.out.println("Lookup each person by name...");

            asList(Madhu.getName(), Mohamed.getName())
